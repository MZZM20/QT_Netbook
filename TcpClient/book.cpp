...
void Book::uploadLargeFileData()
{
//    QFile file(m_strUploadFilePath);
//    qDebug() << m_strUploadFilePath;
//                    ENUM_MSG_TYPE_UPLOAD_LARGEFILE_REQUEST,  //上传超大型文件
    largeFile->seek(m_iRecved);
//    char *pBuffer = new char[4096];
    qint64 ret = 0;
    PDU* pdu = mkPDU(4096);
    ret = largeFile->read((char*)pdu->caMsg, 4096);
    if (ret < 0) {
        QMessageBox::warning(this, "上传文件", "上传文件失败:读文件失败");
        largeFile->close();
        pdu->uiMsgType = ENUM_MSG_TYPE_UPLOAD_LARGEFILE_FAIL;  //上传超大型文件失败提示
    }
    else {
        m_iRecved += ret;
        pdu->uiMsgType = ENUM_MSG_TYPE_UPLOAD_LARGEFILE_REQUEST;
        pdu->uiPDULen = sizeof(PDU) + ret;
        pdu->uiMsgLen = ret;
//        memcpy(pdu->caMsg, pBuffer, ret);
    }
    TcpClient::getInstance().getTcpSocket().write((char*)pdu, pdu->uiPDULen);
//    TcpClient::getInstance().getTcpSocket().waitForBytesWritten();
    qDebug() << "已上传" << m_iRecved << "字节";
    free(pdu);
    pdu = NULL;
//    delete []pBuffer;
//    pBuffer = NULL;
...
void Book::uploadFile()
{
    ...
    if (!m_strUploadFilePath.isEmpty())
    {
        ...
        QString strCurPath = TcpClient::getInstance().curPath();
        qDebug() << strFileName << "  " << strCurPath;
        PDU *pdu = mkPDU(strCurPath.size()+1);
        pdu->uiMsgType = ENUM_MSG_TYPE_UPLOAD_FILE_REQUEST;
        memcpy(pdu->caMsg, strCurPath.toStdString().c_str(), strCurPath.size()+1);
...
//        m_pTimer->start(1000);
        if (fileSize <= LARFEFILE_SIZE) {
            ...
            largeFile = new QFile(m_strUploadFilePath);
