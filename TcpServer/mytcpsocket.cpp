...
//        qDebug() << pdu->uiMsgType;
        ...
        case ENUM_MSG_TYPE_DEL_DIR_REQUEST:
        {
            ...
            else
            {
                ...
                respdu->uiMsgType = ENUM_MSG_TYPE_DEL_DIR_RESPOND;
                memcpy(respdu->caData, DEL_DIR_FAILURED, strlen(DEL_DIR_FAILURED)+1);
            }
        ...
        case ENUM_MSG_TYPE_UPLOAD_FILE_REQUEST:
        {
            ...
            qDebug() << strPath << " " << pPath << " " << caFileName;
            delete []pPath;
            pPath = NULL;
            ...
            if (m_iTotal <= LARFEFILE_SIZE) {
                ...
                qDebug() << "大文件接受模式";
                PDU* respdu = mkPDU(0);
                respdu->uiMsgType = ENUM_MSG_TYPE_UPLOAD_LARGEFILE_RESPOND;
                write((char*)respdu, respdu->uiPDULen);
                free(respdu);
                respdu = NULL;
            }
        ...
        case ENUM_MSG_TYPE_UPLOAD_LARGEFILE_REQUEST:
        {
            PDU *respdu = NULL;
            respdu = mkPDU(0);
//            QByteArray *buff = new QByteArray[pdu->uiMsgLen];
//            memcpy(buff, pdu->caMsg, pdu->uiMsgLen);
            m_file.write((const char *)pdu->caMsg, pdu->uiMsgLen);
//            qDebug() << "here question";
            m_iRecved += pdu->uiMsgLen;
            ...
            else if (m_iTotal < m_iRecved)
            {
                ...
                strcpy(respdu->caData, "continue to translate");
                respdu->uiMsgType = ENUM_MSG_TYPE_UPLOAD_LARGEFILE_RESPOND;  //上传超大型文件回复;
            }
            qDebug() << "文件还剩" << m_iTotal - m_iRecved << "个字节需要传输";
            write((char*)respdu, respdu->uiPDULen);
            free(respdu);
            respdu = NULL;
            break;
        }
        case ENUM_MSG_TYPE_UPLOAD_LARGEFILE_FAIL:
        {
            m_file.close();
            qDebug() << "客户端上传文件到中途失败，本次文件传输终止";
            break;
        }
    ...
    else
    {
        ...
        m_file.write(buff);
        if (onceFileNum--) {
            qDebug() << "断点续传第" << 8 - onceFileNum << "步" ;
            m_file.close();
            while (!m_file.open(QIODevice::Append));
        }
        else onceFileNum = 7;
        m_iRecved += buff.size();
        if (m_iTotal == m_iRecved)
